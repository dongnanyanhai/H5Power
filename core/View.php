...
class View extends Fn_base {
    ...
    // 新增，用以获取网站配置文件config.ini.php内的值
    public $cfg;
    public $iscatid2;
    ...
    private function load_model($name) {
        ...
        if(APP::get_plugin_id()){
            return Controller::plugin_model(APP::get_plugin_id(),$name);
        }else{
    ...
    protected function _listdata($param) {
        ...
        if (isset($system['action']) && $system['action'] == 'position') {
            ...
            //搜索关键字
            if(APP::get_plugin_id()){
                $search = Controller::plugin_model(APP::get_plugin_id(),'search');
            }else{
        ...
        if (isset($system['join']) && $system['join'] && $system['on']) { //JOIN联合查询
            ...
            if(APP::get_plugin_id()){
                $db_join    = Controller::plugin_model(APP::get_plugin_id(),$table_join);
            }else{
        ...
        if (isset($system['more']) && $system['more']) { //附表
            ...
            if ($model) {
                ...
                if(APP::get_plugin_id()){
                    $db_data    = Controller::plugin_model(APP::get_plugin_id(),$table_data);
                }else{
    ...
    protected function relation($id, $num = 10) {
        ...
        if(APP::get_plugin_id()){
            $db   = Controller::plugin_model(APP::get_plugin_id(),'content');
        }else{
