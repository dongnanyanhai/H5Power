...
class HtmlController extends Admin {
	...
	public function categoryAction() {
	    if ($this->isPostForm()) {
			...
			if ($this->site['SITE_EXTEND_ID']) $this->adminMsg(lang('a-sit-29', array('1' => $sites[$this->site['SITE_EXTEND_ID']]['SITE_NAME'])));
	...
	public function formAction() {
		...
		if ($submit) {
			...
			if (!isset($form[$mid]['tablename'])) $this->adminMsg(lang('a-cat-109', array('1' => $mid)));
			...
			if (empty($data)) $this->adminMsg(lang('a-con-107') . '(' . $count . ')', '', 0, 1, 1);
			foreach ($data as $t) {
				if ($this->createForm($mid, $t)) $count++;
			}
			$nextpage  = $page + 1;
			if ($page >= $totalpage) {
				$this->adminMsg(lang('a-con-107') . '(' . $count . ')', '', 0, 1, 1);
	...
	public function indexcAction() {
		...
		$sites	= App::get_site();
		if (count($sites) > 1) {
			$size = file_put_contents(SITE_ROOT . 'cache/index/' . $this->siteid . '.html', ob_get_clean(), LOCK_EX);
			@unlink(SITE_ROOT . 'index.html');
		} else {
			$size = file_put_contents(SITE_ROOT . 'index.html', ob_get_clean(), LOCK_EX);
		}
	...
	private function dir_mode_info() {
        if (strtoupper(substr(PHP_OS, 0, 3)) == 'WIN') {
            /* 测试文件 */
            $test_file = SITE_ROOT . 'finecms_test.txt';
			/* 检查目录是否可读 */
