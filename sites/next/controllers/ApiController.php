...
class ApiController extends Common {
    ...
    public function downAction() {
        ...
        if (strpos($file, ':/')) {
            ...
            $file = strpos($file, '/') === 0 ? SITE_ROOT.$file : $file;
            if (!is_file($file)) {
    ...
    // 动态获取微信js-sdk接口
    public function wxGetConfigAction(){
        // 要验证
        $is_debug = $_GET['isdebug'] ? $_GET['isdebug']: false;

        $url = $_GET['url'] ? $_GET['url'] : ($_SERVER['HTTP_REFERER'] ? $_SERVER['HTTP_REFERER'] : null);
        ...
        echo $js->config(array("onMenuShareTimeline","onMenuShareAppMessage"), (bool)$is_debug, true);
    }
    ...
    public function indexAction() {
        ...
        echo '校正：' . date('Y-m-d H:i:s', @filemtime(SITE_ROOT . 'config/version.ini.php'));
    }
