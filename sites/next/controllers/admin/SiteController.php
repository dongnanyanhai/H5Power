...
class SiteController extends Admin {
	...
	public function addAction() {
	    if ($this->isPostForm()) {
			...
			$site = App::get_site();
			$id   = count($site) + 1;
			if (empty($data['name']) || empty($data['url'])) $this->adminMsg(lang('a-sit-9'));
			$site[$id] = array(
				'DOMAIN'			=> $data['url'],
				'SITE_NAME'			=> $data['name'],
	...
	public function editAction() {
		$id   = (int)$this->get('id');
		$site = App::get_site();
		if (!isset($site[$id])) $this->adminMsg(lang('a-sit-13'));
	    if ($this->isPostForm()) {
			$data = $this->post('data');
			if (empty($data['name']) || empty($data['url'])) $this->adminMsg(lang('a-sit-9'));
	...
	public function delAction($id = 0, $show = 0) {
		if (!auth::check($this->roleid, 'site-del', 'admin')) $this->adminMsg(lang('a-com-0', array('1' => 'site', '2' => 'del')));
		$site = $id ? $id : (int)$this->get('id');
		if ($show && empty($site)) $this->adminMsg(lang('a-sit-28'));
		...
		@unlink(SITE_ROOT . 'cache/index/' . $site . '.html');
	...
	private function copy_table($table, $site, $from = 1) {
		...
		if (empty($table) || $site <= 1 || $this->content->is_table_exists($news)) return false;
