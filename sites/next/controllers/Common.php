...
class Common extends Controller {
    ...
    public function __construct() {
        parent::__construct();
        if (!file_exists(SITE_ROOT . './cache/install.lock')) $this->redirect(url('install/'));
        ...
        if (is_dir(PLUGIN_DIR . 'member') && !empty($this->plugin['member']) && !$this->plugin['member']['disable'] ) {
            $this->member       = $this->plugin_model('member','member');
            
            $this->membergroup  = $this->cache->get('member_membergroup');
            $this->membermodel  = $this->cache->get('member_model_member');
        ...
        if (cookie::is_set('member_id') && cookie::is_set('member_code')) {
            ...
            if (!empty($uid) && $code == substr(md5(SITE_MEMBER_COOKIE . $uid), 5, 20)) {
                ...
                if (is_file(MODEL_DIR . ucfirst(strtolower($member_table)) . 'Model.php') && $_memberinfo && $member_table) {
                    $_member    = $this->plugin_model('member',$member_table);
