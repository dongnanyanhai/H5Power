...
class CategoryModel extends Model {
    ...
    public function getData($site = 0) {
        return $this->where('site=' . $this->getSiteId($site))->order('listorder ASC')->select();
    }
    ...
    public function child($catid, $parent = false, $typeid = 0) {
        ...
        if ($data['child'] && $data['arrchildid']) { //存在子栏目
            if ($parent && ($typeid ? $typeid == $data['typeid'] : true)) $str .= $catid . ',';
            $ids = array();
            $arrchildid = $data['arrchildid'];
            if ($arrchildid) $ids = explode(',', $arrchildid);
            foreach ($ids as $id) {
                $str .= $this->child($id, $parent, $typeid);
            }
        } else {
            if ($typeid ? $typeid == $data['typeid'] : true) $str .= $catid . ',';
        }
        return $str;
    }
    ...
    public function check_catdir($catid = 0, $catdir) {
        if (empty($catdir)) return TRUE;
        $this->where('catdir=?', $catdir);
        if ($catid) $this->where('catid<>?', $catid);
    ...
    public function getParentData($catid) {
        $cat = $this->find($catid);
        if ($cat['parentid']) $cat = $this->getParentData($cat['parentid']);
        return $cat;
    }
    ...
    public function setChildData($parentid, $data) {
        foreach ($data as $catid => $t) {
            if ($t['child']) $data[$catid]['arrchilds'] = $this->getArrchildid($t['catid'], $data);
        }
        return $data;
    }
