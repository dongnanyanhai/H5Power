	...
	public function postAction() {
	    ...
	    if ($this->isPostForm()) {
			...
			if ($data['id']		= $this->form->set(0, $data)) {
				...
				// 增加留言成功发送功能
				// var_dump($this->model);
				$toemail = $this->model['setting']['email'];
				if ($toemail != ""){
					//$toemail = $this->site['SITE_MAIL_USER'];
					$subject = lang('a-fnx-12');
					$message = "<h3>".lang('a-fnx-13')."</h3><dl>";
					foreach ($this->post('data') as $key => $value) {
						$msgext = strtolower(pathinfo($value,PATHINFO_EXTENSION));
						if($msgext == "jpg" || $msgext == "gif" || $msgext == "png" || $msgext == "bmp"){
							$message = $message."<dt>".$this->model['fields']['data'][$key]['name'].":</dt><dd><img src='".SITE_URL.$value."'/></dd>";
						}else if($msgext == "zip" || $msgext == "rar" || $msgext == "pdf" || $msgext == "doc" || $msgext == "ppt" || $msgext == "xls" || $msgext == "docx" || $msgext == "pptx" || $msgext == "xlsx"){
							$filename = pathinfo($value,PATHINFO_BASENAME);
							$message = $message."<dt>".$this->model['fields']['data'][$key]['name'].":</dt><dd><a href='".SITE_URL.$value."'>".$filename."</a></dd>";
						}else{
							$message = $message."<dt>".$this->model['fields']['data'][$key]['name'].":</dt><dd>".htmlspecialchars_decode($value)."</dd>";
						}
					}
					$message = $message."</dl>";
					mail::set($this->site);
					//mail::sendmail($toemail,$subject,$message);
					if (mail::sendmail($toemail,$subject,$message)) {
						$message = lang('a-ind-36');
						//echo $message.'Success';
						//Log::write($message, 'Success');
			        } else {
			        	$message = lang('a-ind-37');
			        	//echo $message.'Error';
