...
class FormController extends Admin {
    ...
    public function __construct() {
		...
		if ($this->join) {
		   ...
		   if ($this->join['typeid'] == 1) $join_info  = '<a href="' . url($this->namespace . '/admin_content/', array('modelid'=>$this->join['modelid'])) . '">' . lang('a-for-4', array('1'=>$this->join['modelname'])) . '</a>';
		}
	...
	public function listAction() {
	    ...
	    $pagelist	= $pagelist->total($total)->url(url($this->namespace . '/admin_form/list', $urlparam))->num($pagesize)->page($page)->output();
		$count		= array();
...
<!-- ' . lang('a-for-15') . ' -->';
	...
	public function addAction() {
		...
		if ($this->isPostForm()) {
			...
			if ($data['id'] = $this->form->set(0, $data)) {
			    ...
			    $this->adminMsg(lang('success'), url($this->namespace . '/admin_form/list', array('modelid' => $this->modelid, 'cid' => $this->cid)), 3, 1, 1);
			} else {
	...
	public function delAction($id=0, $all=0) {
	    ...
	    $all or $this->adminMsg(lang('success'), url($this->namespace . '/admin_form/list', array('modelid' => $this->modelid, 'cid' => $this->cid)), 3, 1, 1);
	}
