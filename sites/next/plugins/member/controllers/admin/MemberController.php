...
class MemberController extends Admin {
    ...
    public function __construct() {
		parent::__construct();
		$this->mgroup = $this->plugin_model('member','member_group');
	}
	...
	public function editAction() {
		...
		$info   = $this->plugin_model('member',$model['tablename']);
		$_data  = $info->find($id);
		if ($this->isPostForm()) {
		    $data = $this->post('data');
		...
		$count[1] = $this->member->count('member', null, 'status=1');
		$count[2] = $this->member->count('member', null, 'status=0');
		$oauth    = $this->plugin_model('member','oauth');
	...
	public function pmsAction() {
	    $type      = $this->get('type');
		$memberpms = $this->plugin_model('member','member_pms');
		switch ($type) {
		    case 'show';
			    $id   = (int)$this->get('id');
			    if ($this->isPostForm()) {
	...
	public function delAction() {
		...
		if ($table) {
			...
			$model = $this->plugin_model('member',$table);
			$model->delete('id=' . $id);
		}
		//删除会员统计表
		$count = $this->plugin_model('member','member_count');
		$count->delete('id=' . $id);
		//删除一键登录
		$oauth = $this->plugin_model('member','oauth');
		$oauth->delete('username=?', $data['username']);
		//删除短消息
		$pms   = $this->plugin_model('member','member_pms');
		$pms->delete("sendid=" . $id . " AND sendname='" . $data['username'] . "'");
		//删除收藏夹
		$favorite = $this->plugin_model('member','favorite');
		...
		if ($model) {
		    foreach ($model as $m) {
			    $db = $this->plugin_model('member',$m['tablename']);
		        $db->delete('userid=' . $id);
			}
		}
		//删除会员扩展
	...
	public function cacheAction($show=0) {
		...
		$this->cache->set('member_membergroup', $cache);
	...
	public function extendAction() {
		...
		$table	= $this->plugin_model('member',$model['tablename']);
		switch ($type) {
		    case 'set':	//参数设置
				if ($this->isPostForm()) {
					...
					$model	= $this->plugin_model('member','model');
