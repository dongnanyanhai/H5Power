...
class SearchModel extends Model {
	...
	private function getSQL($param, $kw, $kw_fields, $kw_or) {
		...
		if (isset($param['modelid']) && $param['modelid'])  $where .= ' AND `' . $this->prefix . $tablename . '`.`modelid`=' . (int)$param['modelid'];
		...
		if ($table_fields) {
		    foreach ($table_fields as $field) {
			    if (isset($param[$field]) && $param[$field]) {
			    	$param[$field] = addslashes($param[$field]);
				    $value = $param[$field];
		...
		if ($data_fields && $table && $more) {
		    foreach ($data_fields as $field) {
			    if (isset($param[$field]) && $param[$field]) { $param[$field] = addslashes($param[$field]);
					$value = $param[$field];
					$andor = is_array($where_or) && in_array($field, $where_or) ? 'OR' : 'AND';
				    if (is_numeric($value)) {
	...
	private function addData($params, $kw, $sql, $modelid, $catid) {
		...
		$total		= count($data);
		if ($total == 1 && $data && strcasecmp($data[0]['title'], $kw) == 0) Controller::redirect($data[0]['url']);
		$ids		= '';
		...
		$data['id'] = strlen($kw) > 1 ? $this->insert($data) : true;
	...
	private function updateData($data, $params, $sql, $modelid, $catid) {
		...
		$kw			= $data['keywords'];
		$total		= count($cdata);
		if ($total	== 1 && $cdata && strcasecmp($cdata[0]['title'], $kw) == 0) Controller::redirect($cdata[0]['url']);
		$ids		= '';
