...
class RelatedlinkController extends Admin {
    ...
    public function __construct() {
		parent::__construct();
		$this->relatedlink = $this->plugin_model($this->namespace,'relatedlink');
	}
	...
	public function indexAction() {
	    if ($this->post('submit_del') && $this->post('form') == 'del') {
			...
			$this->adminMsg($this->getCacheCode('relatedlink') . lang('success'), url($this->namespace . '/admin_relatedlink/'), 3, 1, 1);
	    ...
	    $url      = url($this->namespace . '/admin_relatedlink/index', array('page'=>'{page}', 'kw'=>$kw));
    ...
    public function delAction($id=0, $all=0) {
	    ...
	    $all or $this->adminMsg($this->getCacheCode('relatedlink') . lang('success'), url($this->namespace . '/admin_relatedlink/index'), 3, 1, 1);
	}
	...
	public function importAction() {
	    if ($this->post('submit')) {
	        ...
	        $this->adminMsg($this->getCacheCode('relatedlink') . lang('a-tag-6', array('1'=>$i, '2'=>$k, '3'=>$j)), url($this->namespace . '/admin_relatedlink/index'), 3, 1, 1);
	    }
	...
	public function cacheAction($show=0) {
	    ...
	    $show or $this->adminMsg(lang('a-update'), url($this->namespace . '/admin_relatedlink'), 3, 1, 1);
	}
}