...
class TagController extends Admin {
    ...
    public function __construct() {
		parent::__construct();
		$this->tag = $this->plugin_model($this->namespace,'tag');
	}
	...
	public function indexAction() {
	    if ($this->post('submit_del') && $this->post('form') == 'del') {
			...
			$this->adminMsg($this->getCacheCode('tag') . lang('success'), url($this->namespace . '/admin_tag/'), 3, 1, 1);
	    ...
	    $url      = url($this->namespace . '/admin_tag/index', array('page'=>'{page}', 'kw'=>$kw));
    ...
    public function delAction($id=0, $all=0) {
	    ...
	    $all or $this->adminMsg($this->getCacheCode('tag') . lang('success'), url($this->namespace . '/admin_tag/index'), 3, 1, 1);
	}
	...
	public function importAction() {
	    if ($this->post('submit')) {
	        ...
	        $this->adminMsg($this->getCacheCode('tag') . lang('a-tag-6', array('1'=>$i, '2'=>$k, '3'=>$j)), url($this->namespace . '/admin_tag/index'), 3, 1, 1);
	    }
	...
	public function cacheAction($show=0) {
	    $qok  = $this->get('qok');
		if ($show == 0 && !$qok) $this->adminMsg(lang('a-tag-20'), url($this->namespace . '/admin_tag/cache', array('qok'=>1)), 0, 1, 2);
	    ...
	    $show or $this->adminMsg(lang('a-update'), url($this->namespace . '/admin_tag'), 3, 1, 1);
	}
}