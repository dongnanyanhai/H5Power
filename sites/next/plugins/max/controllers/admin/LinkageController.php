...
class LinkageController extends Admin {
    ...
    public function __construct() {
		parent::__construct();
		$this->link  = $this->plugin_model($this->namespace,'linkage');
		$this->level = 0;
    ...
    public function indexAction() {
	    if ($this->isPostForm()) {
			...
			$this->adminMsg($this->getCacheCode('linkage') . lang('success'), url($this->namespace . '/admin_linkage/'), 3, 1, 1);
		}
	...
	public function listAction() {
		...
		if ($this->isPostForm()) {
			...
			$this->adminMsg($this->getCacheCode('linkage') . lang('success'), url($this->namespace . '/admin_linkage/list', array('keyid'=>$keyid)));
		}
		...
		$pagelist   = $pagelist->total($total)->url(url($this->namespace . '/admin_linkage/list', array('keyid'=>$keyid, 'parentid'=>$parentid, 'page'=>'{page}')))->num($pagesize)->page($page)->output();
		$sdata      = array();
		foreach($result as $t){
			$sdata[$t['id']] = $t;
			$sdata[$t['id']]['option']  = ($parent && $this->isLastNode($t['keyid'], $t['id'])) ? '<a href="' . url($this->namespace . '/admin_linkage/list', array('keyid'=>$keyid, 'parentid'=>$t['id'])) . '">' . lang('a-lin-0') . '</a> | ' : '';
			!admin_auth($this->userinfo['roleid'], 'linkage-addson') or $sdata[$t['id']]['option'] .= '<a href="' . url($this->namespace . '/admin_linkage/addson', array('id'=>$t['id'])) . '">' . lang('a-add') . '</a> | ';
			!admin_auth($this->userinfo['roleid'], 'linkage-editson') or $sdata[$t['id']]['option'] .= '<a href="' . url($this->namespace . '/admin_linkage/editson', array('id'=>$t['id'])) . '">' . lang('a-edit') . '</a> | ';
		...
		if ($pdata) $pstr = "<a href='" . url($this->namespace . '/admin_linkage/list',array('keyid'=>$keyid,'parentid'=>$pdata['parentid'])) . "'>" . lang('a-lin-2') . "</a> | ";
		$str  = "<tr>
	...
	public function addsonAction() {
	    ...
	    if ($this->isPostForm()) {
			...
			$this->adminMsg($this->getCacheCode('linkage') . lang('success'), url($this->namespace . '/admin_linkage/list', array('keyid'=>$keyid, 'parentid'=>$pid)), 3, 1, 1);
		}
		//菜单列表
		$tree    = $this->instance('linkage_tree');
	...
	public function editsonAction() {
	    ...
	    if ($this->isPostForm()) {
			...
			$this->adminMsg($this->getCacheCode('linkage') . lang('success'), url($this->namespace . '/admin_linkage/list', array('keyid'=>$data['keyid'], 'parentid'=>$this->post('parentid'))), 3, 1, 1);
		}
		//菜单列表
		$tree    = $this->instance('linkage_tree');
		$linkage = $this->cache->get('linkage');
	...
	public function delAction() {
		...
		$this->childnode = array();
		$this->link->delete('id=' . $id);
		$this->adminMsg($this->getCacheCode('linkage') . lang('success'), url($this->namespace . '/admin_linkage/list', array('keyid'=>$keyid)), 3, 1, 1);
	}
    ...
    public function cacheAction($show=0, $site_id=0) {
		if ($show == 0 && !$this->get('qok')) $this->adminMsg(lang('a-lin-8'), url($this->namespace . '/admin_linkage/cache', array('qok'=>1)), 0, 1, 2);
