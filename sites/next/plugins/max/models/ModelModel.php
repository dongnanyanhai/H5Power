...
class ModelModel extends Model {
    ...
    public function set($modelid = 0, $data) {
        ...
        $real_table_name =  $data['tablename'];
        //添加模型入库
        $this->insert($data);
        ...
        if ($data['typeid'] != 2 && $data['typeid'] != 4) {
            if ($data['typeid'] == 1) { //内容模型
                ...
                $sql = "CREATE TABLE IF NOT EXISTS `" . $this->prefix . "{table}` (`id` int(10) unsigned NOT NULL AUTO_INCREMENT,`cid` mediumint(8) NOT NULL,`status` tinyint(2) unsigned NOT NULL DEFAULT '1',`updatetime` int(10) unsigned NOT NULL DEFAULT '0',PRIMARY KEY (`id`),KEY `status` (`status`),KEY `updatetime` (`updatetime`),KEY `cid` (`cid`)) ENGINE=MyISAM DEFAULT CHARSET=utf8;";
            }

            $this->query(str_replace('{table}', $real_table_name, $sql)); //创建模型表
            $this->create_model($real_table_name, $data['typeid']); //创建Model文件
            //创建多站点
            $sites  = App::get_site();
            ...
            $sql = "CREATE TABLE IF NOT EXISTS `" . $this->prefix . $real_table_name . "` (`id` MEDIUMINT(8) NOT NULL ,PRIMARY KEY (`id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8;";
            $this->query($sql);
            //创建Model
            $this->create_model($real_table_name, $data['typeid']);
        } elseif ($data['typeid'] == 4) {   //会员扩展模型
            $sql = "CREATE TABLE IF NOT EXISTS `" . $this->prefix . $real_table_name . "` (`id` int(10) NOT NULL AUTO_INCREMENT,`touserid` mediumint(8) NOT NULL,`userid` mediumint(8) NOT NULL,`username` char(20) NOT NULL,`status` tinyint(2) unsigned NOT NULL DEFAULT '1',`inputtime` int(10) unsigned NOT NULL DEFAULT '0', `updatetime` int(10) unsigned NOT NULL DEFAULT '0',`ip` char(20) NULL,`verify` text NOT NULL,PRIMARY KEY (`id`),KEY `status` (`status`),KEY `updatetime` (`updatetime`),KEY `inputtime` (`inputtime`),KEY `userid` (`userid`),KEY `touserid` (`touserid`)) ENGINE=MyISAM DEFAULT CHARSET=utf8;";
            $this->query($sql);
            //创建Model
            $this->create_model($real_table_name, $data['typeid']);
        }
    ...
    public function create_model($table, $typeid) {
        ...
        "    }" . PHP_EOL . PHP_EOL .
        "}";
        file_put_contents(PLUGIN_DIR . APP::get_plugin_id() . DS . 'models' . DS . $table . 'Model.php', $c);
    }
    
    //删除模型文件
    protected function del_model($table) {
        $file  = PLUGIN_DIR . APP::get_plugin_id() . DS . 'models' . DS . ucfirst($table) . 'Model.php';
